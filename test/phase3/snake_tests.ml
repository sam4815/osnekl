...
let%expect_test "Testing [Snake.step] with growth and turn..." =
  let t =
    create ~length:5 |> fun t ->
    grow_over_next_steps t 5 |> fun t ->
    set_direction t Direction.Up |> fun t -> step_n_times t 5
  in
  Stdio.printf !"%{sexp: t option}\n%!" t;
  [%expect
    {|
...
let%expect_test "Testing [Snake.step] with self collision..." =
  ...
  in
  let t =
    create ~length:10 |> fun t ->
    step_n_times t 1 |> fun t ->
    set_direction_if_some t Direction.Up |> fun t ->
    step_n_times t 1 |> fun t ->
    set_direction_if_some t Direction.Left |> fun t ->
    step_n_times t 1 |> fun t ->
    set_direction_if_some t Direction.Right |> fun t -> step_n_times t 1
  in
  Stdio.printf !"%{sexp: t option}\n%!" t;
  [%expect {| () |}]
